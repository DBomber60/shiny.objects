...
Assume we have measurements of binary treatment taken at two times, $(A_1, A_2)$, covariates $(X_t, Z_t)$ measured over time (before $A_1$ and before $A_2$), and a final continuous outcome $Y$ measured at the end of the study. We want to assess the effect of the sequential treatment. We assume that the covariates and outcome are correlated due to unmeasured common causes of each, denoted by $u$. However, that the causal structure of the variables is unknown. In particular, we do not know whether treatment has a direct effect on the intermediate outcomes. We also don't know which variables have a direct effect on the outcome. Therefore, we aim to incorporate the uncertainty in the causal structure into our estimation.
...
where $\phi$ is generated using the posterior predictive distribution.
